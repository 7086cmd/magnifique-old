...
const fileClient = new FileClient({ fileId: id.value, roomId: room.value, messageId: msg.value }, { username: client.value.userId, password: client.value.password }, client.value, refresh.value)
...
fileClient.fetch().then(() => {
  ...
  ext.value = lookup(fileClient.file.name.split('.').reverse()[0], fileClient.file.mime)
  if (fileClient.file.uploader === client.value.userId) {
    align.value = '60%'
    width.value = '80%'
  }
  upld.value = roomperson.value.filter(x => fileClient.file.uploader === x.id)[0].name
  fname.value = '提醒：由于系统原因，中文文件名部分暂时会显示乱码。正在获取文件名中......'
  setTimeout(() => {
    ...
    <el-card :style="('width: ' + width as React.CSSProperties)" shadow="never">
      <template #header>
        <span>
          ...
          于
          <span style="color: #b1b3b8" v-text="dayjs(item.uploadDate).format('YYYY-MM-DD HH:mm:ss')" />
          上传
        </span>
      </template>
      <template #default>
        ...
        <el-button type="success" round plain :icon="Download" @click="fileClient.download()">下载</el-button>
        <el-button type="danger" round plain :icon="Delete" @click="fileClient.delete()">删除</el-button>
      </template>
    </el-card>
  </div>
</template>
