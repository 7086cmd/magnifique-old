...
export class FileClient {
  ...
  fetch = async () =>
    axios(baseurl + "message/upload", {
      params: { id: this.data.fileId },
    }).then(
      (response) => (this.file = response.data.details as UploadFile.Item)
    );
  action = async (method: "post" | "delete") =>
    (
      await axios(baseurl + "message/upload", {
        data: { data: this.data, auth: this.auth },
        method,
      })
    ).data;
  download = async () => {
    ...
    resp.status === "ok" &&
      window.open(
        toPort(baseurl + "file/download?token=" + resp.details.token),
        "_blank"
      );
