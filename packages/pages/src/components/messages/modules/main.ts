...
export class MessageClient {
  ...
  getFullList = async () =>
    (await axios(baseurl + "message/memberlist")).data.details;
  createRoom = async (content: {
    users: string[];
    title: string;
    description: string;
  }) => {
  ...
  fileCenter = {
    ...
    download: async (fileId: string, roomId: string) => {
      ...
      if (result.data.status === "ok") {
        const url = toPort(
          baseurl + "file/download?token=" + result.data.details.token
        );
    ...
    getContent: async (id: string) =>
      (await axios(baseurl + "message/upload", { params: { id } })).data
        .details as UploadFile.Item,
