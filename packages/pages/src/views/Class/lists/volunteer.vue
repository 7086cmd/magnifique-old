...
axios(
  `${baseurl}class/${gradeid}/${classid}/member/get?password=${password}`
).then((response) => {
...
const refresh = () => {
  loading.value = true;
  axios(
    `${baseurl}class/${gradeid}/${classid}/get/volunteer?password=${password}`
  ).then((response) => {
    loading.value = false;
          ...
          <el-card shadow="never">
            ...
            >
              ...
              <el-table-column label="参与者">
                <template #default="props">
                  <el-tag
                    v-if="typeof props.row.person === 'number'"
                    type="success"
