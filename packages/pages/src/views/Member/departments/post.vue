...
const refresh = async () => {
  ...
  const response = await axios(
    `${baseurl}member/post/${number}/work/get/post`,
    {
      params: {
        password,
      },
...
const deletepost = async (props: { row: PostList }) => {
  ...
  const response = await axios(
    `${baseurl}member/post/${number}/work/del/post`,
    {
      data: {
        id: props.row.id,
        password,
...
const download = async (props: { row: PostList }) => {
  const response = await axios(
    `${baseurl}member/post/${number}/work/download/post`,
    {
      method: "post",
      data: {
        id: props.row.id,
  ...
  if (response.data.status === "ok") {
    window.open(
      toPort(
        `${baseurl}member/post/download/${response.data.details.token}/${props.row.title}.docx`
      )
    );
  } else {
    failfuc(response.data.reason, response.data.text);
...
const submitpost = async () => {
  // Write It Again
  const response = await axios(
    `${baseurl}member/post/${number}/work/new/post`,
    {
      data: {
        id: postData.id,
        password,
        ...
        <el-card shadow="never">
          <el-table :data="allData" max-height="640px">
            ...
            <el-table-column align="right" fixed="right">
              <template #header>
                <el-button type="text" @click="newpost = true">
                  新建投稿
                </el-button>
              </template>
              <template #default="props">
                <el-button type="text" size="small" @click="download(props)">
                  下载
                </el-button>
