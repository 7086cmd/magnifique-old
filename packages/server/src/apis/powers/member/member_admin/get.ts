...
import { getMap, getSingleMemberAsRaw } from 'packages/server/src/modules/powers/member'
...
export default async (ctx: Context) => {
  try {
    ...
    if (un?.position.includes('chairman'))
      ...
      ctx.response.body = {
        status: 'ok',
        details: [getMap({ type: 'department', withPosition: true, department: un.department })],
      }
    else
      ctx.response.body = {
        status: 'error',
