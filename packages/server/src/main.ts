...
const httpsServer = createHttpsServer(
  {
    key: readFileSync(
      resolve(tmpdir(), "..", "magnifique", "ssl", "server.key")
    ),
    cert: readFileSync(
      resolve(tmpdir(), "..", "magnifique", "ssl", "server.pem")
    ),
  },
  server.callback()
);
...
const io = new Server(
  process.env.NODE_ENV === "development" ? httpServer : httpsServer,
  {
    cors: {
...
downloadRouter.get("/api/image/download/:fname", async (ctx) => {
  ...
  ctx.response.body = connectionActions.directedDownloadImage(
...
router.get("/api/class/post", async (ctx) => {
  const params = new URLSearchParams(ctx.querystring);
  const password = params.get("password") as string;
  const gradeid = params.get("gradeid") as string;
  const classid = params.get("classid") as string;
  if (
    loginClass(parseInt(gradeid), parseInt(classid), String(password)).status ==
    "ok"
  ) {
    ctx.response.body = postActions.getClass(
      parseInt(gradeid),
      parseInt(classid)
    );
  } else {
...
router.get("/api/class/volunteer", async (ctx) => {
  const params = new URLSearchParams(ctx.querystring);
  const password = params.get("password") as string;
  const gradeid = params.get("gradeid") as string;
  const classid = params.get("classid") as string;
  if (
    ...
    ctx.response.body = volunteerActions.getVolunteerAsClass(
      parseInt(gradeid),
      parseInt(classid)
    );
  } else {
...
router.get(
  "/api/class/graph/:gradeid/:classid/:start/:end/:type/person",
  async (ctx) => {
    try {
      ...
      if (
        ...
        const chartData = deductionActions.graphAsPerson(
          parseInt(gradeid),
          parseInt(classid),
          start,
          end
...
router.get(
  "/api/class/graph/:gradeid/:classid/:start/:end/:type/reason",
  async (ctx) => {
    try {
      const password = getPassword(ctx);
      const { gradeid, classid, start, end } = ctx.params;
      if (
        ...
        const chartData = deductionActions.graphAsReason(
          parseInt(gradeid),
          parseInt(classid),
          start,
          end
...
router.get(
  "/api/class/graph/:gradeid/:classid/:start/:end/:type/date",
  async (ctx) => {
    try {
      const password = getPassword(ctx);
      const { gradeid, classid, start, end } = ctx.params;
      if (
        ...
        const chartData = deductionActions.graphAsDate(
          parseInt(gradeid),
          parseInt(classid),
          start,
          end
...
router.post("/api/class/export/volunteer", async (ctx) => {
  try {
    ...
    if (loginClass(gradeid, classid, password).status == "ok") {
      const token = v4();
      csvTokens[token] = volunteerActions.exportData.exportAsClass(
        gradeid,
        classid,
        config
      );
...
router.get("/api/member/admin/:id/get/:department/volunteer", async (ctx) => {
  try {
    ...
    if (loginMember(parseInt(ctx.params.id), password).status == "ok") {
      if (
        memberActions.memberAdminLimitCheckPower(
          ctx.params.id,
          "member-volunteer"
        )
      ) {
        ctx.response.body = volunteerActions.getVolunteerAsDepartment(
          ctx.params.department
        );
      } else {
        ctx.response.body = {
          status: "error",
...
router.post("/api/member/admin/create/volunteer", async (ctx) => {
  try {
    ...
    if (loginMember(number, password).status == "ok") {
      if (
        memberActions.memberAdminLimitCheckPower(number, "member-volunteer") ||
        memberActions.memberAdminLimitCheckPower(number, "volunteer")
      ) {
        ctx.response.body = volunteerActions.createVolunteerMulti(
          volunteer as VolunteerMulti
        );
      } else {
        ctx.response.body = {
          status: "error",
  ...
  try {
    ...
    if (loginMember(number, password).status == "ok") {
      if (
        ...
        ctx.response.body = volunteerActions.editVolunteerStatusMulti(
          person,
          id
        );
      } else {
    ...
    if (loginMember(number, password).status == "ok") {
      if (
        ...
        if (type === "all") {
          ...
          csvTokens[token] = volunteerActions.exportData.exportAsDepartment(
            department,
            config
          );
        ctx.response.body = {
...
router.post("/api/member/:id/workflow/pause", async (ctx) => {
  try {
    ...
    if (loginMember(parseInt(num), password).status == "ok") {
      ctx.response.body = memberActions.editWorkflow(
        parseInt(num),
        id,
        "planning"
      );
...
router.get("/api/member/:id/volunteer/get", async (ctx) => {
  try {
    ...
    if (loginMember(Number(ctx.params.id), password as string).status == "ok") {
      ctx.response.body = volunteerActions.getVolunteerAsOwn(
...
router.post("/api/member/:id/volunteer/export", async (ctx) => {
  try {
    ...
    if (loginMember(number, password).status == "ok") {
      const token = v4();
      csvTokens[token] = volunteerActions.exportData.exportAsOwn(
        number,
        config
      );
      ctx.response.body = {
...
// 学习部可用API
router.get("/api/member/admin/post", async (ctx) => {
  try {
    const params = new URLSearchParams(ctx.querystring);
    const password = params.get("password") as string;
    const number = Number(params.get("number") as string);
    if (loginMember(Number(number), password).status == "ok") {
      if (memberActions.memberAdminLimitCheckPower(number, "post")) {
...
router.delete("/api/member/admin/post", async (ctx) => {
  try {
    ...
    if (loginMember(Number(number), password).status == "ok") {
      if (memberActions.memberAdminLimitCheckPower(ctx.params.id, "post")) {
        ctx.response.body = postActions.deletePost(Number(person), id);
      } else {
        ctx.response.body = {
          status: "error",
          reason: "no-auth",
...
router.get("/api/member/post", async (ctx) => {
  try {
    const params = new URLSearchParams(ctx.querystring);
    const password = params.get("password") as string;
    const number = Number(params.get("number") as string);
    if (loginMember(number, password).status == "ok") {
      ctx.response.body = postActions.getOwn(number);
    } else {
      ctx.response.body = {
        status: "error",
        reason: "password-wrong",
...
router.put("/api/member/post", async (ctx, next) => {
  await uploader.single("file")(ctx, next);
  if (ctx.file !== undefined) {
    try {
      const { auth } = ctx.request.body as {
        auth: { number: number; password: string };
      };
      const { number, password } = auth;
      if (loginMember(Number(number), password).status == "ok") {
        ctx.response.body = postActions.editLocation(Number(number), ctx.file);
      } else {
...
router.post("/api/member/post", async (ctx) => {
  try {
    const { auth, data } = ctx.request.body as {
      auth: { number: number; password: string };
      data: {
    ...
    if (loginMember(Number(number), password).status == "ok") {
      ctx.response.body = postActions.createPost(Number(number), id, content);
    } else {
      ctx.response.body = {
        status: "error",
        reason: "password-wrong",
...
router.delete("/api/member/post", async (ctx) => {
  try {
    const { auth, data } = ctx.request.body as {
      auth: { number: number; password: string };
      data: { person: number; id: string };
    ...
    const { id } = data;
    if (loginMember(Number(number), password).status == "ok") {
      ctx.response.body = postActions.deletePost(Number(number), id);
    } else {
      ctx.response.body = {
        status: "error",
        reason: "password-wrong",
...
router.delete("/api/admin/post", async (ctx) => {
  try {
    const { password } = ctx.request.body;
    if (loginAdmin(password).status == "ok") {
      ctx.response.body = postActions.deletePost(
        parseInt(ctx.request.body.person),
        ctx.request.body.id
      );
    } else {
      ctx.response.body = {
...
router.get("/api/message/room", async (ctx) => {
  ...
  const password = params.get("password") as string;
  if (
    (connectionActions.loginModule(username, password).status as string) ===
    "ok"
  ) {
    ctx.response.body = {
      status: "ok",
...
router.post("/api/message/room", async (ctx) => {
  ...
  if (
    ...
    ctx.response.body = connectionActions.roomActions.createRoom(
      roomConfig.data
    );
  } else {
    ctx.response.body = {
...
router.delete("/api/message/room", async (ctx) => {
  ...
  if (
    ...
    ctx.response.body = connectionActions.roomActions.deleteRoom(
      roomConfig.data.id
    );
  } else {
    ctx.response.body = {
...
router.put("/api/message/room", async (ctx) => {
  ...
  if (
    ...
    ctx.response.body = connectionActions.roomActions.createMessageRoomEditor(
      roomConfig.data.id,
      roomConfig.data
    );
  } else {
...
router.get("/api/message/message", async (ctx) => {
  ...
  if (
    ...
    ctx.response.body = connectionActions.messageActions.createMessageReader(
      room,
      username
    );
  } else {
...
router.post("/api/message/message", async (ctx) => {
  ...
  if (
    ...
    ctx.response.body =
      connectionActions.messageActions.createTextMessageCreation(
        roomId,
        messageContent
      );
    connectionActions
      .createBody(
        "post",
        createMessageContent.auth.username,
        createMessageContent.data.roomId
      )
      .forEach((item) => io.emit("message", item));
  } else {
    ctx.response.body = {
      status: "error",
      reason: "password-wrong",
...
router.delete("/api/message/message", async (ctx) => {
  ...
  if (
    (connectionActions.loginModule(auth.username, auth.password)
      .status as string) === "ok"
  ) {
    ctx.response.body = connectionActions.messageActions.createMessageDeletion(
      data.roomId,
      data.messageId,
      auth.username
    );
    connectionActions
      .createBody("delete", auth.username, data.roomId)
      .forEach((item) => io.emit("message", item));
  } else {
    ctx.response.body = {
      status: "error",
...
router.patch("/api/message/message", async (ctx) => {
  ...
  if (
    ...
    ctx.response.body =
      connectionActions.messageActions.createTextMessageUpdate(
        data.roomId,
        data.messageId,
        data.updateContent,
        auth.username
    ...
    connectionActions
      .createBody("patch", auth.username, data.roomId)
      .forEach((item) => io.emit("message", item));
  } else {
    ctx.response.body = {
      status: "error",
...
router.put("/api/message/upload", async (ctx, next) => {
  ...
  if (ctx.file !== undefined) {
    if (
      (connectionActions.loginModule(auth_username, auth_password)
        .status as string) === "ok"
    ) {
      ctx.response.body = connectionActions.uploadedFileHandler(
        ctx.file,
        auth_username,
        data_roomId,
        data_withMessage !== "false",
        data_filename
      );
    } else {
      ctx.response.body = {
...
router.post("/api/message/upload", async (ctx) => {
  const { auth, data } = ctx.request.body as {
    ...
    data: { roomId: string; fileId: string };
  };
...
router.get("/api/message/upload", async (ctx) => {
  ...
  ctx.response.body = {
    status: "ok",
    details: connectionActions.createUploadedFileItemReader(fileId as string),
  };
});
