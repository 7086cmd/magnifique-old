...
const getPerson = (personId: string) => {
  ...
  if (result?.type === "admin") return "管理员";
    ...
    return getSingleMemberAsRaw(result.number).details?.name;
  else if (result?.type === "class")
    return getClassName(result.gradeid, result.classid);
  else return "";
};
...
const readMyRooms = (user: string) => {
  ...
  return roomIds
    .map((item) => {
      ...
      return {
        title:
          roomInf.config.users.length === 1
            ? `我（${getPerson(user)}）`
            : roomInf.config.users.length !== 2
            ? roomInf.config.title
            : getPerson(
                roomInf.config.users.filter((item) => item !== user)[0]
              ),
        id: item,
        recent: getRecent(createSingleRoomItemGetter(item)),
        members: createSingleRoomItemGetter(item).config.users.map((x) => ({
          id: x,
          name: getPerson(x),
        })),
    ...
    .sort((item1, item2) => {
      const i1rec = messageActions.createMessageReader(
        item1.id,
        item1.members.map((item) => item.id)[0]
      ).details;
      const i2rec = messageActions.createMessageReader(
        item2.id,
        item2.members.map((item) => item.id)[0]
      ).details;
      const i1s = i1rec?.sort(
        (a, b) => dayjs(b.createDate).unix() - dayjs(a.createDate).unix()
      )[0];
      const i2s = i2rec?.sort(
        (a, b) => dayjs(b.createDate).unix() - dayjs(a.createDate).unix()
      )[0];
