...
export const getUploadedFile = (
  ...
  const vail2 = roomActions
    .createSingleRoomItemGetter(roomId)
    .config.users.includes(username);
  if (vail1 && vail2) {
    Object.entries(createSingleRoomItemGetter(roomId).details).forEach(
      ([index, value]) => {
        if (value.type === "file" && value.content === id)
          recievementActions.single(roomId, index, username);
      }
    );
    return {
      status: "ok",
