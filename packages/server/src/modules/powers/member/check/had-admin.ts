...
export default (memberNumber: string | number, dutyWhichIsWaitingForCheck: 'post' | 'deduction' | 'volunteer' | 'radio' | 'member' | 'member-volunteer' | 'member-register') => {
  if (getSingleMemberAsRaw(Number(memberNumber)).status === 'error') {
    ...
    if (dutyWhichIsWaitingForCheck === 'member') {
      ...
      const details = getSingleMemberAsRaw(Number(memberNumber)).details as unknown as member
      return details.union.position.includes('minister') || details.union.position.includes('chairman')
    } else {
