...
const getDepartmentClass = (gradeid: number, classid: number, department: string) => {
  const depMembers = getDepartment(department).details
  return depMembers.filter(member => member.number.toString().startsWith((createYearTransformer(gradeid) * 100 + classid).toString()))
}
