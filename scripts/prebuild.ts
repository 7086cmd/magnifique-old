...
const lintFile = async () => {
        ...
        for (let j = 0; j in results[i].messages; j++) {
            if (results[i].messages[j].severity == 1) {
                console.log(
                    '  ' +
                        chalk.dim(
                            `${results[i].messages[j].line}:${results[i].messages[j].column}`
                        ) +
                        '  ' +
    ...
    if (hasw) {
        console.log(
            chalk.bold.yellow(
                `✖ ${tw + te} problems (${te} errors, ${tw} warnings)`
            )
        )
    }
    if (te > 0) {
...
const build = async () => {
    ...
    await buildDocs(resolve(__dirname, './docs')).then(() => {
        ...
        const folderlist = readdirSync(
            resolve(__dirname, './docs/.vitepress/dist'),
            {
                withFileTypes: true,
            }
        )
        const childfolderlist = readdirSync(
            resolve(__dirname, './docs/.vitepress/dist/assets'),
            {
                withFileTypes: true,
            }
        )
        for (let i = 0; i in folderlist; i++) {
            if (folderlist[i].isFile() == true) {
                try {
                    writeFileSync(
                        resolve(__dirname, './dist/docs', folderlist[i].name),
                        readFileSync(
                            resolve(
                                __dirname,
                                './docs/.vitepress/dist',
